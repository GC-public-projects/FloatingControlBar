...
- `acquireImage(...)` : Private function that retrieves the image from the imageReader buffer, create a bitmap from it, save it to the device and open it in the "ScreenScreenshot" screen from the MainActivity (thanks to the extras).
...
### Purposes
- open the different screens of the app following instructions or not
...
- `showPermissionDialog`, `modifyShowPermissionDialog`, `openOverlaySettings()` : these components are part of the overlay mechanism and already explained in the `FloatingButton` project of my Github


- `companion object` : hold the string constants used as extra of the intent of this MainActivity. 
...
- `onCreate` : retrieve of the intent of the MainActivity in order go get the extra if some exist and open the correct screen in function. The extra `INTENT_EXTRA_SCREENSHOT_PATH` is ...
